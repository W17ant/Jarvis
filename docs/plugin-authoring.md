...
**By voice (fastest).** Say *"hey jarvis, build me a plugin called weather-pro that fetches NOAA forecasts. The tool should be called fetch_noaa, and I'll say 'noaa weather' to invoke it."* Jarvis calls the `build_plugin` tool, scaffolds the manifest + handler stub, hot-loads it, and confirms the tool is callable. Open `bridge/plugins/<slug>/handler.mjs` and replace the stub `run()` body with real logic — the watcher hot-reloads on save.

If you also describe the desired behaviour ("fetch the NOAA hourly endpoint and return the next 6 hours"), Jarvis switches to code-agent mode: scaffolds the manifest and seeds the handler, then asks for a second confirmation before calling `code_agent_run` to fill in the real logic. End-to-end voice → working plugin in ~2 minutes.

**By hand.** Read on. The voice path uses the same manifest schema documented below; anything you can do by hand, you can do by voice.
